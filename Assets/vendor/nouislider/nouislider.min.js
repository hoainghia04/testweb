...
!function(a) {
    ...
    function O(a, b) {
        if (!g(b) && !Array.isArray(b))
            throw new Error("noUiSlider (" + Z + "): 'padding' option must be numeric or array of exactly 2 numbers.");
        ...
        if (0 !== b) {
            ...
            if (a.padding[0] < 0 || a.padding[1] < 0)
                throw new Error("noUiSlider (" + Z + "): 'padding' option must be a positive number(s).");
    ...
    function X(a, e, g) {
        ...
        function z(a, b, c) {
            ...
            if ("count" === a) {
                if (b < 2)
                    throw new Error("noUiSlider (" + Z + "): 'values' (>= 2) required for mode 'count'.");
                var d = b - 1
                  , e = 100 / d;
                for (b = []; d--; )
                    b[d] = d * e;
        ...
        function F(a, b, c, d) {
            var f = function(f) {
                return !!(f = G(f, d.pageOffset, d.target || b)) && (!(ra.hasAttribute("disabled") && !d.doNotReject) && (!(n(ra, e.cssClasses.tap) && !d.doNotReject) && (!(a === oa.start && void 0 !== f.buttons && f.buttons > 1) && ((!d.hover || !f.buttons) && (qa || f.preventDefault(),
        ...
        function L(a, b) {
            b.handle && (m(b.handle, e.cssClasses.active),
            ua -= 1),
            b.listeners.forEach(function(a) {
                za.removeEventListener(a[0], a[1])
            }),
            0 === ua && (m(ra, e.cssClasses.drag),
            _(),
            a.cursor && (Aa.style.cursor = "",
            Aa.removeEventListener("selectstart", c))),
        ...
        function U(a, b, c, d, f, g) {
            return ka.length > 1 && (d && b > 0 && (c = Math.max(c, a[b - 1] + e.margin)),
            f && b < ka.length - 1 && (c = Math.min(c, a[b + 1] - e.margin))),
            ka.length > 1 && e.limit && (d && b > 0 && (c = Math.min(c, a[b - 1] + e.limit)),
            f && b < ka.length - 1 && (c = Math.max(c, a[b + 1] - e.limit))),
            e.padding && (0 === b && (c = Math.max(c, e.padding[0])),
    ...
    function Y(a, b) {
        if (!a || !a.nodeName)
            throw new Error("noUiSlider (" + Z + "): create requires a single element, got: " + a);
        if (a.noUiSlider)
